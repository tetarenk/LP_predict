...
OUTPUT: (1) File summary of simulation results
        (2) File summary of available, used, and unused time in each weather band through simulation
        (3a) Histograms displaying unused LST range per weather band
        (3b) Histograms displaying remaining MSB LST range per weather band
...
def read_cal(table,cal_table):
    ...
    NOTE - this calibrator method is not currently implemeted.'''
...
def elevationcheck(jcmt,mjd,target):
    ...
    and adjusts the elevation limit for these sources.'''
...
def predict_time(sim_start,sim_end,wvmfile,LAPprograms,Block,path_dir,flag,total_observed,FP,m16al001_tally,\
    ...
    for k in range(0,len(obs_mjd)):
        ...
        if len(targets)>0:
            ...
            '''(optional) schedule calibrators every hour -- Method not currently implemented.
